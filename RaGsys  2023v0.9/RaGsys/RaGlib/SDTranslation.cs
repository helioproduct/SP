...
public class Symbol : ICloneable {
    ...
    public class LLTranslator {
        ...
        public string Print_Res(List<Symbol> list)    // вывод вызовов до операционного символа
        {
            string S = "";
            ...
            S += list[list.Count - 1].Value;
            return S;

        }

        public void Print_Stack(Stack<Symbol> stack)   // вывод стека
        {
        ...
        public bool checkEndofRule(Symbol symbol)    // проверка, что данный операционный символ лежит в самом конце правой части правила продукции
        {
            foreach (var temp in G.Prules)
            {
                for (int i = 0; i < temp.RightChain.Count; ++i)
                {
                    if (temp.RightChain[i] == symbol)
                    {
                        if (i == temp.RightChain.Count - 1)
                        {
                            return true;
                        }
                    }
        ...
        public (List<Symbol>, bool) Rules(Symbol symbol, String S)    //нахождение правил продукций вида symbol -> SABC | AB | e
        {
            ...
            if (symbol.Value == " ")
            {
                return (new List<Symbol>(), false);
            }

            foreach (var temp in G.Prules)
            {
                List<Symbol> rule = new List<Symbol>();
                if (temp.LeftNoTerm == symbol)
                {
                    int count = 0;
                    foreach (var t in temp.RightChain)
                    {
                        if (S == t)
                        {
                            ++count;
                        ...
                        else
                        {
                            if (G.T.Contains(t))
                            {
                                flag = false;     // проверка на наличие операции, не находящейся в просмотре входной цепочки
                            }
                        }
                        if (t == Symbol.Epsilon)
                        {
            ...
            if (ind > -1)
            {
                return (rules[ind], true);   // выбранно правило с нужным операционным символом на входной цепочке
            }
            if (flag && rules.Count() > 0)
            {
                return (rules[0], true);   // не было найденно правило с операционным символом на входной ленте
            }

            if (flag_e)
            {
                return (new List<Symbol>(), true);  // правая часть представляет собой только эпсилон-правило
            }
            return (new List<Symbol>(), false);   // не было найдено вхождений

        }
        public bool ParseReq(List<Symbol> input)
        {
            ...
            do
            {
                ...
                if (G.T.Contains(curStackSymbol) && curStackSymbol == curInputSymbol)    // операционный символ
                {
                    ++i;
                    curInputSymbol = input[i];
                    res.Add(curStackSymbol);
                    ans.Add(Print_Res(res));
                    if (checkEndofRule(curStackSymbol))    // возврат на пути вызовов на два уровня вверх
                    {
                ...
                {                                                                 // нетерминал
                    ...
                    bool flag;
                    (list, flag) = Rules(curStackSymbol, curInputSymbol.ToString());   // выборка правил продукций для данного нетерминала и проверка на 
                    Stack.Pop();
                    if (flag)
                    {
                        if (list.Count == 0)   // не было найдено правил продукций
                        {
                            res.RemoveAt(res.Count - 1);
                            continue;
                        }
                        res.Add(curStackSymbol.Value);  // запись нетерминала в путь вызова
                        for (int j = list.Count - 1; j >= 0; j--)
                        {
                            if (list[j].Value != "")
                            {
                                Stack.Push(list[j].Value);
                            }
                        }
                    }
                    else
            ...
            } while (Stack.Peek() != Symbol.Sentinel);

            if (curInputSymbol != Symbol.Sentinel)   // проверка на пустоту стека
            {
                return false;
            }

            Console.WriteLine("Путь вызовов нетерминалов для каждого операционного символа: ");


            foreach (var s in ans)
            {
                Console.Write("Операционный символ {" + s[s.Length - 1] + "} обработан данным путем: ");
                Console.WriteLine(s);
            }
