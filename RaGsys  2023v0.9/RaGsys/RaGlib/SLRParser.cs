   ...
   public class SLRGrammar : Grammar
    {
        ...
        public List<List<State>> Closures { get; set; }
        ...
        public bool IsLR0Grammatic()
        {
            foreach (var closure in Closures)
            {
                var rules = closure.ToArray();


                for (int ruleIdx = 0; ruleIdx < rules.Length; ruleIdx++)
                {
                    var rule = rules[ruleIdx];
                    if (rule.GetRHSymbol(P) == Symbol.Epsilon)
                    {

                        for (int ruleIdx1 = ruleIdx + 1; ruleIdx1 < rules.Length; ruleIdx1++)
                        {
                            var rule1 = rules[ruleIdx1];
                            if (rule1.GetRHSymbol(P) == Symbol.Epsilon || char.IsLower(rule1.GetRHSymbol(P).symbol[0]) || signs.Contains(rule1.GetRHSymbol(P).symbol[0]))
                            {
                                Console.WriteLine($"Найдены правила в позиции {ruleIdx + 1} и {ruleIdx1 + 1} (CLOSURE {Closures.IndexOf(closure)})");
                                return false;
                            }
                            var rightSymbol = rule1.GetRHSymbol(P).symbol[0];
                            if (char.IsUpper(rightSymbol))
                            {
                                var lrParser = new MyLRParser(this);
                                var oFirst = lrParser.OFirst(rightSymbol.ToString());
                                if (!oFirst.Contains("e"))
                                {
                                    Console.WriteLine($"Найдены правила в позиции {ruleIdx + 1} и {ruleIdx1 + 1} (CLOSURE {Closures.IndexOf(closure)})");
                                    return false;
                                }
                    ...
                    else if (rule.GetRHSymbol(P) == "_")
                    {
                        Console.WriteLine("Замечена грамматика с эпсилон правилом!");
                        return false;
                    }
                }
